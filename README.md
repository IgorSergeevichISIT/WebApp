...
Отчеты по лабораторным работам и ДЗ отправлять на репозиторий и задеплоить проект на любой хост, который захотите [КАК ЗАДЕПЛОИТЬ ПРОЕКТ](https://www.netlify.com/](https://www.youtube.com/watch?v=uLp-zgset00&t=49s)
...
Основной вариант лабораторных по бэкенду - это Django и `Python`. Но можно выполнять также на `Java`, `C#` и `Node.js`, при выполнении условия лабораторных работ. Для фронтенда только `React`+`Redux`+`axios`+`React-Bootstrap`
